...
import 'package:flutter_ahlib/src/util/mime.dart';
      ...
      try {
        ...
        if (option!.headTimeout != null) {
          future = future.timeout(
            option.headTimeout!,
            onTimeout: () => throw const DownloadException._head('timed out'),
          );
        }
        resp = await future;
      } catch (e) {
      ...
      switch (await option!.whenOverwrite(filepath)) {
        ...
        case OverwriteBehavior.addSuffix:
          for (var i = 1;; i++) {
            ...
            var extension = path_.extension(filepath);
  ...
  try {
    ...
    try {
      ...
      if (option.downloadTimeout != null) {
        future = future.timeout(option.downloadTimeout!, onTimeout: () => throw const DownloadException._download('timed out'));
      }
      resp = await future;
    } catch (e) {
